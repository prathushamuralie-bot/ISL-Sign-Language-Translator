...
================================================================================
Idhu app FAST aagum - camera nerudiya access pannuthu (browser illama),
so delay/lag PROBLEM ELLAM SOLVE aagidum.

Design: Dark black + Golden theme, 3D-style bordered cards, vertical rectangle
history cards.

Install pannunga munnadi: pip install customtkinter pillow
"""
...
from grammar_correct_offline import correct_grammar_offline   # ADD THIS LINE
try:
    import pythoncom
    HAS_PYTHONCOM = True
except ImportError:
...
ALPHABET_CONFIDENCE_THRESHOLD = 45.0  # letters ku konjam low threshold - gestures similar aana confidence kammiya varum
HOLD_DURATION = 1.2
...
def draw_hand_landmarks(frame, hand_landmarks_list):
    ...
    # Gold-ish landmark color (BGR format for opencv: gold ~ (0, 200, 230))
...
class Card3D(ctk.CTkFrame):
    """
    Oru 'shadow' frame + 'top' frame combine panni, 3D depth illusion create pannurom.
    Idhu ippo parent oda FULL SPACE ah fill pannum (fixed size illa) - screen edhu size
    aayirundhalum, automatic ah adjust aagum.
    """
...
class SignLanguageApp:
    def __init__(self, root):
        ...
        # Screen size ku automatic ah fit pannurom - window cut aagama irukka
        ...
        try:
            self.root.state('zoomed')  # Windows la full-screen maximize pannum
        except Exception:
            try:
        ...
        self.letter_prediction_history = []  # jitter smooth panna, last few frames track pannurom
    ...
    def build_ui(self):
        ...
        # ===== Main body - grid layout, camera ku periya space (weight 3) =====
        body = ctk.CTkFrame(self.root, fg_color=BG_BLACK)
        body.pack(fill="both", expand=True, padx=30, pady=20)
        body.grid_columnconfigure(0, weight=3)   # camera - periya pakkam
        body.grid_columnconfigure(1, weight=2)   # message/history
        body.grid_rowconfigure(0, weight=1)

        # ---- Left: Camera 3D card (fills all available space) ----
        ...
        # Card3D uses place() for its children by default (fills parent) - but idhu
        # oru fixed-content card aanadhala, ippo natural height ku vidurom:
        ...
        # ---- Language selector row (grid: 3 columns x 2 rows, always fits) ----
        ...
        # Spelling buffer strip - spell mode ON aana mattum kaanum
        ...
        # Idha default ah pack pannama vidurom - spell mode ON pண்ணும்போது mattum kaatuvom
        ...
        # Scrollable vertical rectangle cards
    ...
    def add_history_card(self, text, timestamp):
        """Vertical rectangle 3D card - ovvoru communicated sentence ku."""
        ...
        # Gold accent strip on the left (vertical rectangle feel)
    ...
    def open_text_to_sign(self):
        ...
        def show_signs():
            ...
            # Vocabulary la irukura words ah, typed text kulla thedurom (longest phrase first,
            # "Good Morning" mathiri rendu-word signs correct ah match aaganum)
    ...
    def update_frame(self):
        ...
        if success:
            ...
            # Video label oda ACTUAL current size ah edukkurom - screen edhu size
            # aayirundhalum, camera ah andha full space ah fill pannum
            self.video_label.update_idletasks()
            target_w = self.video_label.winfo_width()
            target_h = self.video_label.winfo_height()
            if target_w < 50 or target_h < 50:
                target_w, target_h = 640, 480  # layout innum render aagama irundha, fallback
    ...
    def confirm_spelled_word(self):
        if not self.spelled_letters:
            return
        word = "".join(self.spelled_letters).capitalize()  # "KAVI" -> "Kavi", TTS engine
        # ah normal word ah pronounce panna help pannum (ALL CAPS na letter-by-letter spell pannum)
    ...
    def _process_letter_prediction(self, prediction, confidence):
        """Spell mode la, letters ah hold-to-confirm pannitu, buffer la add pannurom."""
        LETTER_HOLD = 0.9  # letters ku konjam fast ah hold pannalam (words vida)

        # ---- Jitter smoothing: last 5 raw predictions vachu, majority ah edukurom ----
    ...
    def speak_sentence(self):
        if not self.sentence:
            return
        translated_words = [translate_word(w, self.current_language) for w in self.sentence]
        display_text = " ".join(translated_words)
        timestamp = datetime.now().strftime("%I:%M %p")
        self.add_history_card(display_text, timestamp)
        # English word keys ah pass pannurom, andha per-word offline audio files play pannurom
        threading.Thread(target=self._speak_thread, args=(list(self.sentence), self.current_language)).start()

    def _speak_thread(self, words, lang_code):
        for word in words:
            if word in SIGN_TRANSLATIONS:
                # Vocabulary word - pre-generated multi-language offline audio use pannurom
                safe_name = word.replace(" ", "_")
                ...
                # Spelled/custom word (e.g. "RAVI") - Windows PowerShell built-in speech use
                # pannurom. Idhu ORU PUTHU, isolated process la run aagum ovvoru word kum -
                # adhaal pyttsx3 oda COM/threading flakiness (sometimes silent ah fail aagum
                # problem) completely avoid aagum. Konjam slow (process start aagum), aana
                # 100% reliable ah work pannum.
                self._speak_offline_windows(word.title())

    @staticmethod
    def _speak_offline_windows(text):
        # Text ulla double-quotes/backticks irundha, PowerShell command break aagum -
        # andha characters ah safe ah clean pannurom
